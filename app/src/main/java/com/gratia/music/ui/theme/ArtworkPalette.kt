 ...
 *
 * Source: Gratia ui/theme/ArtworkPalette.kt
 * https://github.com/kushagrasinghx/Gratia (GPLv3)
 *
 * Modifications for Gratia: Coil 3 -> Coil 2 request API; the original
 * AppSettings.reduceAnimation -> Gratia's SettingsDataStore.reduceAnimationFlow;
 * the dark/light question is answered from GratiaTheme.colors.isDark rather
 * than MaterialTheme luminance; artwork sizing uses Gratia's own artwork
 * size constants; seed-of extraction, the HSL derivation and the crossfade
 * policy are carried over unchanged.
 */
...
/**
 * The colours an artist or album page paints itself in.
 *
...
data class ArtworkPalette(
    ...
    companion object {
        /** The palette before any artwork has been read: Gratia's own theme. */
        fun fallback(dark: Boolean): ArtworkPalette = if (dark) {
            ArtworkPalette(
                background = darkGratiaColors.background,
                wash = darkGratiaColors.background,
                elevated = darkGratiaColors.surface,
                accent = darkGratiaColors.accent,
                onBackground = darkGratiaColors.textPrimary,
                onBackgroundVariant = darkGratiaColors.textSecondary,
                divider = darkGratiaColors.glassBorder,
            )
        } else {
            ArtworkPalette(
                background = lightGratiaColors.background,
                wash = lightGratiaColors.background,
                elevated = lightGratiaColors.surfaceHover,
                accent = lightGratiaColors.accent,
                onBackground = lightGratiaColors.textPrimary,
                onBackgroundVariant = lightGratiaColors.textSecondary,
                divider = lightGratiaColors.glassBorder,
            )
        }
    }
}
 ...
 * off [seedCache] — a page opened from a screen it shares a cover with, or a
 * page opened twice, has nothing to wait for and nothing to fade. Only a
 * sleeve genuinely being seen for the first time starts from the theme's own
 * colours and warms into the artwork's, so it never flashes a placeholder
 * tint. "Reduce animation" turns that crossfade into a cut.
 */
@Composable
fun rememberArtworkPalette(
    imageUrl: String?,
    dark: Boolean = GratiaTheme.colors.isDark,
    /**
    ...
    val reduceAnimation by rememberReduceAnimation()
    ...
    LaunchedEffect(imageUrl, artPx) {
        ...
        val request = ImageRequest.Builder(context)
            ...
            // then shares a memory/disk-cache entry with the header image
    ...
    val target = seed?.toPalette(dark) ?: ArtworkPalette.fallback(dark)
...
/** Compose-facing handle on Gratia's reduce-animation setting. */
@Composable
private fun rememberReduceAnimation(): androidx.compose.runtime.State<Boolean> {
    val context = LocalContext.current
    val flow = remember(context) { SettingsDataStore(context).reduceAnimationFlow }
...
private fun seedOf(bitmap: Bitmap): Seed? {
    ...
    // monochrome sleeve is the entire image.
