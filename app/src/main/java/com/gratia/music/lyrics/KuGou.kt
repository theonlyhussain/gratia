...
object KuGou : LyricsProvider {
    ...
    override suspend fun fetchLyrics(
        ...
        val lines = LrcLib.parseLrc(lrc).takeIf { it.isNotEmpty() } ?: return@withContext null
        
        LyricsResult(
            text = lines.toLrc(),
            syncLevel = SyncLevel.LINE, // KuGou only supports LINE sync
            providerName = name,
            matchConfidence = 80, // Has duration matching, but title/artist based
            durationDifferenceMs = 0L // Approximation
        )
    }
