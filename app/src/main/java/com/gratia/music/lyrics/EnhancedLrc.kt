...
object EnhancedLrc {
    ...
    fun parse(lrc: String): List<LyricLine> {
        val rows = lrc.lineSequence()
            ...
            .map { match ->
                Row(
                    startMs = stamp(match.groupValues[1], match.groupValues[2], match.groupValues[3]),
        ...
        return rows.mapIndexedNotNull { index, row ->
            if (row.words.isEmpty()) {
                ...
                return@mapIndexedNotNull if (text.isEmpty()) null else LyricLine(row.startMs, text)
            }
            ...
            val lineEnd = rows.getOrNull(index + 1)?.startMs
            ...
            LyricLine(
                startMs = minOf(row.startMs, words.first().startMs),
    ...
    private class Row(val startMs: Long, val words: List<MatchResult>, val plain: String)
