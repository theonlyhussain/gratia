 ...
 *
 * googlevideo paces a continuous response down to roughly playback speed: ask
 * for a whole track in one request and it arrives at about 15 kB/s, barely ahead
 * of the rate it is being consumed at. Ask for the same bytes as bounded ranges
 * and each one is served at line rate — measured on the same track, over the
 ...
 * can only just keep up, and it is the reason a remote track can resolve
 * perfectly — a valid URL, no error anywhere — and still produce no sound at
 * all: the bytes arrive, but never faster than they are played, so playback
 * opens and then sits in BUFFERING for as long as the connection is asked to
 * cooperate.
 *
 ...
 * already bounded — a seek, or anything else that names a range — is passed
 * straight through untouched rather than chunked twice.
 */
@UnstableApi
class ChunkedDataSource(
    ...
    override fun open(dataSpec: DataSpec): Long {
        ...
        if (dataSpec.length != C.LENGTH_UNSET.toLong() || total == null) {
            ...
            // so every other source comes through this branch — and a refusal
            // that is neither logged nor handed anywhere surfaces as a bare
            ...
            try {
                ...
                if (e !is InterruptedIOException) {
                    ...
                    Log.w(TAG, "$who refused the stream: ${e.message}")
                    report(dataSpec, e)
                }
                throw e
            }
    ...
    private fun openChunk() {
        ...
        try {
            ...
            // the time this surfaces as a playback error.
            if (e !is InterruptedIOException) {
                Log.w(
                    TAG,
                ...
                // The only place a refusal of the *real* fetch is seen. Left
                // here it is just a failed track; handed back, it is the one
                // piece of evidence that retiring the client rests on.
                report(spec, e)
            }
            throw e
        }
    ...
    override fun read(buffer: ByteArray, offset: Int, length: Int): Int {
        ...
        // A range that ends early is re-opened for the part that didn't arrive,
        // which is also how the step to the next range happens. The attempt
        // limit is what stops a server that has decided to send nothing from
