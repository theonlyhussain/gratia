 ...
 *
 * Source: Gratia data/sources/StreamFormat.kt (SourceStream.kt, MusicSource.kt,
 * SourceHealth) — https://github.com/kushagrasinghx/Gratia (GPLv3)
 *
 * Modifications for Gratia: [MusicSource] is asked for Gratia's [RemoteTrack]
 * rather than the original Song model, and track identity rides on Gratia's
 * RemoteTrack.videoId; documentation carried over. Everything else — the
 * StreamFormat rules, the StreamRequest tiers and the SourceHealth distinction
 * between Unreachable and Rejected — is the original implementation.
 */
    ...
    val summary: String
        get() = if (isDolbyAtmos) {
            ...
            listOfNotNull(
                ...
                bitDepth?.let { "$it-bit" },
                sampleRateHz?.let {
                    String.format(Locale.ROOT, "%.1f", it / 1000f).removeSuffix(".0") + " kHz"
                },
 ...
 * the request that asks for it. Sources that don't care leave it empty.
 */
data class SourceStream(
     ...
     * track that doesn't — but it is worth knowing, because the reason is
     ...
     * [QualityUpgrade][com.gratia.music.provider.sources.QualityUpgrade].
     */
     ...
     * whoever queued the track, which is not the same evidence.
     */
     ...
     * second look during playback can leave the source already serving the
     * track out of the search: asking it again for the same recording at the
     * same tier is deterministic and only ever reproduces the stream already
     * playing.
     */
 ...
 *
 * Not a quality *setting* — the setting lives in Gratia's settings and is
 * turned into one of these per request, because the answer depends on the
 * connection in hand at the moment a track starts, not on what was chosen in
 * Settings an hour ago on a different network.
 */
 ...
 * that refused the credentials is neither.
 */
 ...
 *
 * All three methods run on IO and may throw; [SourceResolver] is the only
 * caller, and treats a throw as "this one didn't work" rather than letting it
...
interface MusicSource {
    ...
    /** What the user named it, falling back to the service's own name. */
    val displayName: String

    /** Whether this kind can hold bit-exact copies at all. */
    val canServeLossless: Boolean
        get() = kind.canServeLossless

    /** Reachability and credentials, for skipping dead sources. */
    suspend fun health(): SourceHealth

    /**
     * Tracks matching [query], as [RemoteTrack]s already tagged with this
     * source's id so that playing one comes back here.
     *
     ...
     */
    suspend fun search(query: String, limit: Int = 25, waitForAll: Boolean = false): List<RemoteTrack>
